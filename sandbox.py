...
segments.groupby('osm_name')[['step_dist_f', 'step_elevation_f']].sum().sort_values(by='step_dist_f', ascending=False)
m = gpxa.viz.make_chunk_map(segments)
segments.explore(column='coast_speed_mph', tooltip=['coast_speed_mph'], cmap="Reds", vmin=12, vmax=35, m=m)